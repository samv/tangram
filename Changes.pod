...
This release introduces no new features or bugfixes; it is pure
refactoring.
...
Rename many classes; see the source for L<Tangram::Compat> for the
full list.  The L<Tangram::Compat> module handles this renaming.

=back

=head1 RELEASE 2.09, slated for October 2005

This release is a major re-release of Tangram, which is mostly
API-backwards compatible.

However, the following caveats need to be considered carefully;
...
However, so long as you C<use Tangram::Core> (or C<use Tangram>)
before any of these old names are used, then the L<Tangram::Compat>
class should be able to catch the inclusion and load the correct
module instead.

=back

=head2 DEV. RELEASE 2.09_01, xx Aug 2005

=over

=item *

Many, many module renames and splitting of combined modules into
seperate files.  Currently the full list is visible in the source of
L<Tangram::Compat>.
...
Unless critical bugs are reported, the latest 2.08_XX release will be
released as 2.09 in September.
