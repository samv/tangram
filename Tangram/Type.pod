...
C<string>, C<int>, C<real>: see L<Tangram::Scalar>

C<date>, C<time>, C<datetime>, C<dmdatetime>: see L<Tangram::DateTime>

=item B<Compound Structure types>

C<perl_dump>: see L<Tangram::PerlDump>.  A C<perl_dump> structure may
contain any structure which Data::Dumper can dump successfully.

C<storable>: see L<Tangram::Storable>.  Very much like C<perl_dump>,
but implemented via the `Storable' serialisation engine.

C<yaml>: see L<Tangram::YAML>.  Very much like C<perl_dump>, but
implemented via the `YAML' serialisation engine.

=item B<`Flat' Array & Hash types>

Note: these are only actually required if you need to be able to query
on individual fields inside the array/hash - otherwise, the C<perl_dump>
mapping is a lot faster and more flexible.

C<flat_array>: see L<Tangram::FlatArray>

C<flat_hash>: see L<Tangram::FlatHash>

=item B<References to other objects>

C<ref>: see L<Tangram::Ref> (implementing an B<N to 1> relationship,
...
C<set>: see L<Tangram::Set> (implementing an I<unordered> B<N to N>
relationship, with all objects sharing a common base class)

C<iset>: see L<Tangram::IntrSet> (implementing an I<unordered> B<1 to
...
C<array> : see L<Tangram::Array> (implementing an I<ordered> B<N to N>
relationship, with all objects sharing a common base class)

C<iarray>: see L<Tangram::IntrArray> (implementing an I<ordered> B<1
...
C<hash> : see L<Tangram::Hash> (implementing a I<keyed> B<N to N>
relationship, with all objects sharing a common base class)

C<ihash>: see L<Tangram::IntrHash> (implementing a I<keyed> B<1 to N>
relationship, with all objects sharing a common base class)
