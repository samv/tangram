...
documented here; see L<member hash> for a list of predefined
persistent types.
...
Field C<members> contains a reference to the L<member hash>.
...
together inside the member hash, where the typetag is used as a
