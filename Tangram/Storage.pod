...
In either case, the retrieved objects satisfy $filter and are
...
Directives can be passed as key/value pairs. Currently Tangram
...
C<order> is an array of Expr; order result set by increasing value of Exprs.

C<desc> is a boolean; a true value reverses the order specified by the
C<order> option.
