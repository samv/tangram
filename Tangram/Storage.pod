...
=head1 MEMORY MANAGEMENT (IMPORTANT)
...
=head2 connect( $schema, $data_source, $username, $auth, \%options )
...
C<dialect> contains a Dialect object (see L<Tangram::Dialect>); this is
useful for accessing extra functionality available in some dialects of
SQL. You may also specify the dialect as a class name.
...
=head2 select( $remote, opt1 => val1 [, op2 => val2 ... ] )

Returns a list containing all the objects that are type-compatible
with $remote.

A series of optional directives can be passed as key/value
pairs. Currently Tangram recognizes the following directives:
...
Option C<filter> specifies a Filter that can be used to restrict the
result set.

Option C<distinct> specifies that each object in the result set must
be unique (Tangram generates a SELECT DISTINCT).

Option C<order> is an array of Expr; order result set by increasing
value of Exprs.

Option C<desc> is a boolean; a true value reverses the order specified
by the C<order> option.
...
=head2 cursor( $remote, opt1 => val1 [, op2 => val2 ... ] )
...
For options C<filter>, C<order> and C<desc>, see C<select> above.
