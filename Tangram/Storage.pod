...
Option C<order> is an array of Expr; retrieved objects will be ordered
according by increasing order for each Expr.
...
For options C<filter> and C<order>, see C<select> above.
...
Drops references to persistent objects present in memory.
...
objects present in memory (see L<reset>).
