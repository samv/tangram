...
persistent objects. For instance:
    ...
    my $mars = Foo::Location->new( name => 'Mars'); # instantiate the obj
    $storage->insert($mars);                        # add it to the DB
...
of objects and how they behave depends a lot on the method used to
create this relationship - whether it's a C<ref>, an C<array>,
etc -- see Tangram::Schema and Tangram::Type for more information.

Filters can also include boolean expressions:

    my $r_user = $storage->remote('My::Users');
    my @active_premium_users = $storage->select( $r_user,
        filter => (
                   ($r_user->{is_logged_in} eq 'Y') &    # "&" is not a typo
                   ($r_user->{is_premium} eq 'Y' )
                  )
    );

This select retrieves all the users currently logged in who also have a
premium account. Note the use of B<&> instead of B<&&> (or B<and>) - this is
due to a problem in the way Perl handles operator overloading.
For the basic boolean operators, use C<&> as AND, C<|> as OR and C<!> as NOT.
...
Note that it's not yet possible to create an empty filter in this
way -- but we are working on it C<:)>
