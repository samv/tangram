...
would order DESC all the fields mentioned in the order. Passing a

                      desc => 0

would order all the fields ASC.

To specify which fields should order DESC and which ones should
order ASC, pass an array ref to desc, like this:
...
would order field1 and field3 descending, and field2 ascending.
